...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, bridge: SH5DdsBridge, camera_paths: dict):
    ...
    model_path = "/home/rokey/dev_ws/models/test_vision_slot1_1_r.pth"
    ...
    print("  1 : 1번 슬롯(오른팔)으로 분류 명령 하달")
    print("  2 : 2번 슬롯(왼팔)으로 분류 명령 하달")
    print("  3 : 3번 슬롯(오른팔)으로 분류 명령 하달")
    print("  4 : 4번 슬롯(왼팔)으로 분류 명령 하달")
    print("")
    print("[기타 제어]")
    ...
    box_spawn_pos = torch.tensor([1.5, 0.0, 0.85]) # 벨트 높이에 맞춤
    conveyor_speed = 0.3 # 0.3 m/s
    box_phase = "MOVING"
    stop_timer = 0.0
    
    # [최적화] UsdPhysics API를 사용하여 Box의 Kinematic 상태 동적 제어
    from isaacsim.core.utils.stage import get_current_stage
    from pxr import UsdPhysics
    stage = get_current_stage()
    box_prim = stage.GetPrimAtPath("/World/envs/env_0/Box")
    box_rb_api = UsdPhysics.RigidBodyAPI(box_prim) if box_prim.IsValid() else None
    
    # 박스를 처음에 Kinematic(물리 연산 끄기)으로 설정하여 가볍게 만듭니다.
    if box_rb_api:
        box_rb_api.GetKinematicEnabledAttr().Set(True)
        
    rack_relative_offset = None
    ...
    while simulation_app.is_running():
        ...
        # ==========================================
        # [Step 1] 연속 공정 컨베이어 벨트 및 스포너 로직
        # ==========================================
        if "box" in scene.keys():
            box_state = scene["box"].data.root_state_w.clone()
            x_pos = box_state[0, 0].item()
            device = box_state.device
            
            if box_phase == "MOVING":
                if x_pos > 0.70:
                    box_state[0, 0] -= conveyor_speed * sim_dt
                    scene["box"].write_root_state_to_sim(box_state)
                else:
                    box_phase = "STOPPED"
                    print(f"\n[Conveyor] 📦 띠딕! 상자가 작업 영역(X={x_pos:.2f})에 도착했습니다. (물리 연산 OFF 상태 대기)")
            
            elif box_phase == "STOPPED":
                # 로봇의 손(End-effector) 위치가 상자에 가까워지면 충돌 판정(Dynamic) 켜기
                if "robot" in scene.keys():
                    # 임시로 로봇 손의 대략적인 X 위치가 0.6 이상(상자에 접근)일 때 켬
                    robot_x = scene["robot"].data.root_state_w[0, 0].item()
                    # 또는 AI 추론이 활성화되었을 때 켜기
                    if inference_active:
                        print("[Physics] ⚡ 로봇이 접근합니다! 상자 충돌 판정(Dynamic) ON")
                        if box_rb_api: box_rb_api.GetKinematicEnabledAttr().Set(False)
                        box_phase = "PICKING"
                
                # 테스트용: 스페이스바나 키보드로 강제 텔레포트 및 랙 적재 시퀀스
                if time.time() - stop_timer > 3.0 and not inference_active:
                    pass # 대기
                    
            elif box_phase == "PLACED":
                # 작업대에 놓인 후에는 작업대와 함께 움직이도록 고정 (Kinematic ON)
                if "rack" in scene.keys() and rack_relative_offset is not None:
                    rack_state = scene["rack"].data.root_state_w[0]
                    box_state[0, 0:3] = rack_state[0:3] + rack_relative_offset
                    scene["box"].write_root_state_to_sim(box_state)
        ...
        if key in ['1', '2', '3', '4']:
            ...
            # Dummy Teleport 테스트: 강제로 슬롯에 박어넣고 PLACED 상태로 전환
            if "box" in scene.keys() and "rack" in scene.keys():
                target_pose = SLOT_TARGETS.get(active_slot_id if active_slot_id else 1, SLOT_TARGETS[1])
                default_state = scene["box"].data.root_state_w.clone()
                default_state[0, 0:3] = torch.tensor(target_pose, device=device)
                scene["box"].write_root_state_to_sim(default_state)
                
                rack_state = scene["rack"].data.root_state_w[0]
                rack_relative_offset = default_state[0, 0:3] - rack_state[0:3]
                
                if box_rb_api: box_rb_api.GetKinematicEnabledAttr().Set(True)
                box_phase = "PLACED"
                print(f"[Physics] 🛑 상자가 슬롯에 안착되었습니다. (작업대에 종속됨, 물리 연산 OFF)")
                
        elif key == 'r':
            # 컨베이어로 새 상자 스폰
            if "box" in scene.keys():
                box_state = scene["box"].data.root_state_w.clone()
                box_state[0, 0:3] = box_spawn_pos.to(device)
                scene["box"].write_root_state_to_sim(box_state)
                if box_rb_api: box_rb_api.GetKinematicEnabledAttr().Set(True)
                box_phase = "MOVING"
                rack_relative_offset = None
                print("[Conveyor] 🔄 새로운 상자를 스폰합니다!")
        ...
        if inference_active and active_slot_id is not None:
            ...
            if "robot" in scene.keys() and scene["robot"].data.root_state_w is not None:
                robot_pose = scene["robot"].data.root_state_w[0, :7].detach().cpu().numpy()
            else:
                robot_pose = np.zeros(7, dtype=np.float32)
                
            if "box" in scene.keys() and scene["box"].data.root_state_w is not None:
                box_pose = scene["box"].data.root_state_w[0, :7].detach().cpu().numpy()
            else:
                box_pose = np.zeros(7, dtype=np.float32)
            ...
            # 추론 시에는 정확한 진행률(progress)과 페이즈를 알 수 없으므로 근사치 사용
            progress = np.array([50.0], dtype=np.float32)
            ...
            if len(action_execution_queue) > 0:
                ...
                # [수정] action은 66차원. 0~62는 조인트, 63~65는 cmd_vel
                joint_targets = action[:63]
                cmd_vel = action[63:66]
                
                joint_names = scene["robot"].data.joint_names
                with bridge.lock:
                    for i, name in enumerate(joint_names):
                        if i < len(joint_targets):
                            bridge.pending_positions[name] = joint_targets[i]
                    bridge.latest_cmd_vel = (cmd_vel[0], cmd_vel[1], cmd_vel[2])
