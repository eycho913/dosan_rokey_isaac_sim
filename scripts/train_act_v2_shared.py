...
class VisionACTSequenceDataset(Dataset):
    def __init__(self, dataset_dir, context_len=10, chunk_size=20):
        ...
        self.index_map = []    # 데이터가 없을 때를 대비한 빈 리스트 초기화
        ...
        for file_path in hdf5_files:
            ...
            try:
                with h5py.File(file_path, 'r') as f:
                    ...
                    # [공유용 버전] 최대 50개의 에피소드만 랜덤으로 선택하여 로드합니다.
                    MAX_EPISODES = 50
                    if len(demo_keys) > MAX_EPISODES:
                        demo_keys = demo_keys[:MAX_EPISODES]
                    
                    print(f"[INFO] {filename} 에서 {len(demo_keys)}개의 랜덤 에피소드를 로드합니다 (공유용 제한 적용).")
...
def train(args):
    ...
    # num_workers=8로 늘려서 HDF5 병목 현상(I/O)을 획기적으로 해결!
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=0, pin_memory=True)
