...
class HDF5Player:
    ...
    def get_episode(self, ep_idx):
        ...
        if 'obs/images' in demo:
            images = {}
            for cam_name in demo['obs/images'].keys():
                images[cam_name] = demo['obs/images'][cam_name][:]
            data['images'] = images
...
def run_replay(sim, scene, player, start_episode):
    ...
    while simulation_app.is_running():
        ...
        # Display images via OpenCV if available
        if 'images' in ep_data:
            img_list = []
            for cam in ["Left Camera", "TopView", "Right Camera"]:
                if cam in ep_data['images']:
                    img = ep_data['images'][cam][frame_idx]
            ...
            if img_list:
                ...
                cv2.imshow("Vision Data (Left | Top | Right)", combined_img)
                cv2.waitKey(1)
            ...
            if "robot" in scene.keys() and box_pos is not None:
                ...
                if len(scene.finger_indices) > 0:
                    ...
                    if min_dist < 0.15 and finger_target_avg > 0.20:
                        ...
                        scene["box"].write_root_pose_to_sim(target_state[:, :7])
                    else:
