...
class VisionACTSequenceDataset(Dataset):
        ...
        for file_path in hdf5_files:
            ...
            try:
                ...
                for demo_name in sorted(f['data'].keys()):
                    ...
                    # [고속 테스트용] 각 파일당 20개의 에피소드만 로드하여 학습 속도를 10배 향상!
                    if len([ep for ep in self.episodes if ep[0] == file_path]) >= 20:
                        break
    ...
    def __getitem__(self, idx):
        ...
        # Load only the current frame's images to save memory/compute
        f = self.hdf5_handles[file_path]
...
def train(args):
    ...
    # Reduced batch size for Vision ACT to prevent OOM
    ...
    for epoch in range(args.epochs):
        ...
        for batch_states, batch_actions, batch_phases, batch_images, batch_slot_ids in dataloader:
