...
def _setup_cameras():
    ...
    left_cam_path = _find_camera_prim_by_name(stage, "Left_Camera")
    if left_cam_path:
        cam_paths["Left Camera"] = left_cam_path
    else:
        print("[WARNING] Left_Camera not found in stage!")
        
    right_cam_path = _find_camera_prim_by_name(stage, "Right_Camera")
    if right_cam_path:
        cam_paths["Right Camera"] = right_cam_path
    else:
        print("[WARNING] Right_Camera not found in stage!")
...
def replay_and_capture():
    ...
    if args_cli.output_dir.endswith('.hdf5'):
        os.makedirs(os.path.dirname(args_cli.output_dir), exist_ok=True)
    else:
    ...
    if os.path.isfile(args_cli.input_dir) and args_cli.input_dir.endswith('.hdf5'):
        files = [args_cli.input_dir]
    else:
        files = glob.glob(os.path.join(args_cli.input_dir, "*.hdf5"))
        
    files = [f for f in files if "vision" not in os.path.basename(f)]
    
    for input_file in files:
        filename = os.path.basename(input_file)
        if args_cli.output_dir.endswith('.hdf5'):
            output_file = args_cli.output_dir
