...
def run_replay(sim, scene, player, start_episode):
    ...
    def reset_to_frame0():
        ...
        # 로봇 루트 위치 (kinematic body → pose만 설정, 속도 세팅 불가)
        ...
        # 상자 위치 (kinematic body → pose만 설정)
        if "box" in scene.keys():
            bp = ep_data['box_pose'][0]
            bpose = scene["box"].data.default_root_state[:, :7].clone()
            bpose[0, 0:3] = torch.tensor(bp[:3])
            bpose[0, 3:7] = torch.tensor(bp[3:7])
            scene["box"].write_root_pose_to_sim(bpose)
